...
import { Transaction, TransactionList } from './model';
import { ObservableUtilities } from '../common/utilities';

@Injectable()
export class TransactionService {
    private _uri = "/api/transaction";
    ...
    create (transaction: Transaction): Observable<Transaction> {
        ...
        return this._http.post(this._uri, JSON.stringify(transaction))
            .map(this._observable.json)
            .catch(this._observable.error);
    }

    retrieve (id: string = ''): Observable<Transaction> {
        //noinspection TypeScriptUnresolvedFunction
    ...
    update (transaction: Transaction): Observable<Transaction> {
        //noinspection TypeScriptUnresolvedFunction
        return this._http.put(`${this._uri}/${transaction._id}`, JSON.stringify(transaction))
            .map(this._observable.json)
            .catch(this._observable.error);
    }
    ...
    retrieveRange (list: TransactionList): Observable<TransactionList> {
        let options = {
            url: this._uri,
        ...
        options.headers.set('Range', `transactions=${from}-${to}`);
